...
def internet():   
    fp=open('total_infor1.txt','r+',encoding='utf-8')
    lines=fp.readlines()
    select = Choices.get()
...
'''
def mplscheck():
    fp=open('mpls_infor.txt','r+',encoding='utf-8')
    lines=fp.readlines()
    select = Choices.get()
    if (select == bigcstoresChoices[0]):
        ip_mpls1=lines[0].split(",")[0]
        ip_mpls2=lines[0].split(",")[1]
        store=lines[0].split(",")[5]
        ...
        dv = ConnectHandler(**dv)
        dv.enable()
        ping_mpls1 = dv.send_command("ping "+ip_mpls1+" repeat 10")
        ping_mpls2 = dv.send_command("ping "+ip_mpls2+" repeat 10")

        #print(ping_cmd)
        ping_result1 = ping_mpls1.splitlines()[3].split()
        ping_result2 = ping_mpls2.splitlines()[3].split()
        packet_loss1 = ping_result1[5].replace(",","").replace(",","")
        packet_loss2 = ping_result2[5].replace(",","").replace(",","")
        latency1 = ping_result1[9]
        latency2 = ping_result2[9]
        mpls_check_result = '01. Store: '+store+'\n02. IP Address Primary Line: '+ip_mpls1+'\n02.1. Status: '+packet_loss1+'\n02.2. Latency min/avg/max: '+latency1+' ms\n02.3. Output Command: \n'+ping_mpls1+'\n03. IP Address Secondary Line: '+ip_mpls2+'\n03.1. Status: '+packet_loss2+'\n03.2. Latency min/avg/max: '+latency2+' ms\n03.3. Output Command: \n'+ping_mpls2
        #print(mpls_check_result)
        print("Application is running.....")
        Output.insert(END, mpls_check_result) 
...
internet_button = Button(button_frame, height = 2, width = 10, bg="black", fg="white" , text = "Internet\nInformation", command = lambda:internet())
internet_button.grid(row=0, column=0, padx=10)

mpls_button = Button(button_frame, height = 2, width = 10, bg="black", fg="white" , text = "Mpls\nInformation", command = lambda:mpls())
mpls_button.grid(row=1, column=0, padx=10)


internet_check = Button(button_frame, height = 2, width = 10, bg="black", fg="white", text="Internet\nChecker", command= lambda:internetcheck())
internet_check.grid(row=0, column=1, padx=10)

mpls_check = Button(button_frame, height = 2, width = 10, bg="black", fg="white", text="Mpls\nChecker", command= lambda:mplscheck())
...
loadingLabel = Label(root, text='App developed by |> Nguyen Luu Trung', font=('Agency FB',14), bg="orange")
panel1.pack()
Output.pack(pady=15)
button_frame.pack()
